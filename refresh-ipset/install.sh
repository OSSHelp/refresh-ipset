...
mkdir -p "${tmp_dir}" && \
  fetch_files "${tmp_dir}" && \
    install_files "${tmp_dir}" && \
      show_notice "Script ${script_name} (v$ipsver) was installed to ${script_path}."
test -d /etc/cron.daily/ && {
    ln -sf "${script_path}/${script_name}" /etc/cron.daily/refresh-ipset && \
    show_notice "Enabled daily launch from /etc/cron.daily/refresh-ipset."
}
